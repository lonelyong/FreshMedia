...
namespace libMedia.Mp3
{
    static class mp3FrameFactory
    {
        ...
        static public mp3Frame CreateFrame( Stream stream, uint remainingBytes )
        {
            ...
            mp3FrameHeader header = CreateHeader( stream, remainingBytes );
            if( header == null )
                return null;
            ...
            mp3Frame firstTry = new mp3Frame(stream, header, frameFullSize, remainingBytes);
        ...
        /// <returns>wrapper for derived type of AudioFrame</returns>
        static public mp3Frame CreateFrame( byte[] sourceBuffer )
        {
            mp3Frame firstTry = new mp3Frame( sourceBuffer );
        ...
        private static mp3Frame CreateSpecialisedHeaderFrame( mp3Frame firstTry )
        {
            ...
            if( firstTry.IsXingHeader )
            {
                ...
                //else
                return new mp3XingFrame( firstTry );
            }
            else if( firstTry.IsVbriHeader )
                return new mp3VbriFrame( firstTry );
            else
                return firstTry;
        }
        ...
        static mp3FrameHeader CreateHeader( Stream stream, uint remainingBytes )
        {
            ...
            do
            {
                ...
                mp3FrameHeader parsedHeader = new mp3FrameHeader( ReadHeader( stream ) );
