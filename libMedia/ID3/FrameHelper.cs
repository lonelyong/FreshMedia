...
using libMedia.ID3.Exceptions;
using libMedia.ID3.Frames;
        ...
        public FrameBase Build(string frameId, ushort flags, byte[] buffer)
        {
            ...
            if (frame.Compression == true)
            {
                switch (Version)
                {
                    case 3:
                        {
                            size = Utils.Swap.UInt32(reader.ReadUInt32());
                            break;
                        }
                    case 4:
                        {
                            size = Utils.Swap.UInt32(Utils.Sync.UnsafeBigEndian(reader.ReadUInt32()));
                            break;
                        }
                    default:
                        {
            ...
            if (frame.Unsynchronisation == true)
            {
                ...
                size = Utils.Sync.Unsafe(stream, memoryStream, size);
                index = 0;
        ...
        public byte[] Make(FrameBase frame, out ushort flags)
        {
            ...
            if (frame.Compression == true)
            {
                switch (Version)
                {
                    case 3:
                        {
                            writer.Write(Utils.Swap.Int32(buffer.Length));
                            break;
                        }
                    case 4:
                        {
                            writer.Write(Utils.Sync.UnsafeBigEndian(Utils.Swap.UInt32((uint)buffer.Length)));
                            break;
                        }
                    default:
                        {
            ...
            if (frame.Unsynchronisation == true)
            {
                ...
                Utils.Sync.Unsafe(memoryStream, synchStream, (uint)memoryStream.Position);
