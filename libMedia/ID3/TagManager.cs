    ...
    public static class TagManager
    {
        ...
        public static TagModel Deserialize(Stream stream)
        {
            ...
            if (frameModel.Header.Unsync == true)
            {
                ...
                id3TagSize -= Utils.Sync.Unsafe(stream, memory, id3TagSize);
            ...
            while (index < rawSize)
            {
                ...
                uint frameSize = Utils.Swap.UInt32(reader.ReadUInt32());
                index += 4; // have read 4 bytes
                // ID3v2.4 now has sync-safe sizes
                if (frameModel.Header.Version == 4)
                    frameSize = Utils.Sync.Unsafe(frameSize);
                ...
                ushort flags = Utils.Swap.UInt16(reader.ReadUInt16());
        ...
        public static void Serialize(TagModel frameModel, Stream stream)
        {
            ...
            foreach (FrameBase frame in frameModel)
            {
                ...
                if (frameModel.Header.Version == 4)
                    frameSize = Utils.Sync.Safe(frameSize);

                writer.Write(Utils.Swap.UInt32(frameSize));
                writer.Write(Utils.Swap.UInt16(flags));
                writer.Write(buffer);
            }
            ...
            if (frameModel.Header.Unsync == true)
                id3TagSize += Utils.Sync.Safe(memory, stream, id3TagSize);
            else
